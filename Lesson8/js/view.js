// файл який генерує інтерфейс

const contactList = {
    render(contacts, element) { // генерує інтерфейс
    ...
    renderWithTemplate(contacts, element, template) {
        let html = "";
        contacts.forEach(contact => {
            html += Mustache.render(template, contact); // mustache допомагає коду щоб в ньому записались дані з об'єкту і створився html
        });
