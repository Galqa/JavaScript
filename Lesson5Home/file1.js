...
const getBMICategory = (bmi) => {
    ...
    if (bmi >= 18.5 && bmi < 24.9) return 'Нормальна вага';
    if (bmi >= 25 && bmi < 29.9) return 'Надмірна вага';
    return 'Ожиріння';
};

// функція для запиту даних користувача та виведення результатів
const getUserBMI = () => {
    const weight = parseFloat(prompt('Введіть вашу вагу в кілограмах:'));
    const height = parseFloat(prompt('Введіть ваш зріст в метрах:'));
    ...
    alert(`Ваш ІМТ: ${bmi.toFixed(2)} (${category})`);
};

// виклик функції для обчислення ІМТ користувача
getUserBMI();
